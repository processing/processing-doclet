...
public class TemplateWriter extends BaseWriter {
  ...
  private Boolean write(
    ...
    String[] templateFile = FileUtils.loadStrings(
      Shared.i().TEMPLATE_DIRECTORY() + templateName
    );
  ...
  public String writeLoop(
    String templateName,
    ArrayList<HashMap<String, String>> varSet
  ) {
  ...
  public ArrayList<String> writeLoopSyntax(
    String templateName,
    ArrayList<HashMap<String, String>> varSet
  ) {
    String[] templateFile = FileUtils.loadStrings(
  ...
  public ArrayList<String> writeFields(
    String templateName,
    ArrayList<HashMap<String, String>> varSet
  ) {
    String[] templateFile = FileUtils.loadStrings(
  ...
  private String writeLine(
    ...
    for (String key : map.keySet()) {
      if (line.contains(key)) {
        ...
        if (value.equals("")) { //remove html around things that are absent (like images)
          while (line.contains(requireStart)) {
            String sub = line.substring(
              line.indexOf(requireStart),
              line.indexOf(requireEnd) + requireEnd.length()
            );
          ...
          while (line.contains(requireAbsenceStart)) {
            String sub = line.substring(
              line.indexOf(requireAbsenceStart),
              line.indexOf(requireEnd) + requireEnd.length()
            );
            line = line.replace(sub, "");
          }
