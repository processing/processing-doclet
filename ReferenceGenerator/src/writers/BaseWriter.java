...
public class BaseWriter {
  ...
  public static final String jsonDir =
  ...
  protected static String getAnchor(Element element) {
    ...
    if (!Shared.i().isCore(element)) {
      ...
      if (pkg.equals("data/") || pkg.equals("opengl/")) ret = ret; else ret =
        pkg + ret;
    }
  ...
  protected static String getReturnTypes(ExecutableElement element) {
    ...
    if (element.getEnclosingElement() != null) {
      for (Element subElement : element
        ...
        if (
          methodElement.getSimpleName().equals(element.getSimpleName()) &&
  ...
  protected static String longestText(Element element) {
    ...
    for (Element subElement : element
      ...
      .getEnclosedElements()) {
      if (
        (Shared.i().isMethod(element) && !Shared.i().isMethod(subElement)) ||
      ...
      for (JSONObject parameter : ret) {
        ...
        if (!desc.endsWith(": ")) {
          ...
          for (JSONObject parameter2 : ret) {
            ...
            if (
              parameter2.getString("name").equals(parameter.getString("name"))
            ) {
              ...
              for (int i = 0; i < paramArray.length(); i++) {
                for (int j = 0; j < paramArray2.length(); j++) {
                  if (
                    !paramArray.getString(i).equals(paramArray2.getString(j))
                  ) {
  ...
  public static String getSubcategory(String webref) {
    String subcategory;
    if (webref.split(":").length > 1) subcategory =
      webref.split(":")[1]; else subcategory = "";
    return subcategory;
  }
}
